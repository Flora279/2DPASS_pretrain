...
from network.spvcnn_MAE import get_model as SPVCNN
...
class Backbone3DTrainer(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        scheduler = {
            ...
            'interval': 'step' if self.config['train_params'][
    ...
    def on_validation_epoch_end(self):
        loss_mean = torch.stack(self.eval_step_outputs['loss_list']).to(self.device).mean() # mean of single gpu data
        loss_mean = self.all_gather(loss_mean).mean()      # mean of different gpus
...
if __name__ == '__main__':
    ...
    tb_logger = pl_loggers.TensorBoardLogger(log_folder, name=f'{configs.train_params.log_dir}/{configs.dataset_params.mask_ratio}', default_hp_metric=False)
    os.makedirs(f'{log_folder}/{configs.train_params.log_dir}/{configs.dataset_params.mask_ratio}', exist_ok=True)
    profiler = SimpleProfiler(dirpath=f'{log_folder}/{configs.train_params.log_dir}/{configs.dataset_params.mask_ratio}',
    ...
    if not configs['test']:
        ...
        shutil.copy('main_3d_MAE.py', backup_dir)
        shutil.copy('network/voxel_fea_generator_MAE.py', backup_dir)
        ...
        shutil.copy('network/spvcnn_MAE.py', backup_dir)
    ...
    trainer = pl.Trainer(
        ...
        logger=tb_logger,
        gradient_clip_val=10 # clip_grad_norm_(model.parameters(), optim_cfg.GRAD_NORM_CLIP) norm=10
    )
