...
@register_dataset
class SemanticKITTI_fraction(data.Dataset):
    def __init__(self, config, data_path, imageset='train', num_vote=1):
        with open(config['dataset_params']['label_mapping'], 'r') as stream:
            semkittiyaml = yaml.safe_load(stream)
        print('enter SemanticKITTI_fraction')
        self.config = config
        self.num_vote = num_vote
        self.learning_map = semkittiyaml['learning_map']
        self.imageset = imageset
        self.skip_ratio = config['dataset_params']['skip_ratio']
        ...
        for i_folder in split:
            self.im_idx_full += absoluteFilePaths('/'.join([data_path, str(i_folder).zfill(2), 'velodyne']), num_vote)

        if imageset == 'train':
            for count, file_path in enumerate(self.im_idx_full):        # avoid randomization, 保证每次training set fraction里的scan恒定
                if count % self.skip_ratio == 0:
                    self.im_idx.append(file_path)
        elif imageset == 'val':         # use full data for validation
            self.im_idx = self.im_idx_full
        print('len(self.im_idx)', len(self.im_idx), self.im_idx[10])
        # if imageset == 'val':
        #     self.im_idx = self.im_idx[:100]
        # else:
        #     self.im_idx = self.im_idx[:100]
