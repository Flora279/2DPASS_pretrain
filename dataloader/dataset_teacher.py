...
class point_image_dataset_semkitti(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        student_xyz = self.student_aug(xyz.copy())
        student_xyz = np.clip(student_xyz, self.coors_range_xyz[:, 0], self.coors_range_xyz[:, 1])        
        student_feat = np.concatenate((student_xyz, sig), axis=1)

        teacher_xyz = self.teacher_aug(xyz.copy())
        teacher_xyz = np.clip(teacher_xyz, self.coors_range_xyz[:, 0], self.coors_range_xyz[:, 1])        
        teacher_feat = np.concatenate((teacher_xyz, sig), axis=1)

        data_dict = {}
        data_dict['student_point_feat'] = student_feat
        data_dict['teacher_point_feat'] = teacher_feat
...
def collate_fn_default(data):
    ...
    student_points = [torch.from_numpy(d['student_point_feat']) for d in data]
    teacher_points = [torch.from_numpy(d['teacher_point_feat']) for d in data]
    labels = [torch.from_numpy(d['point_label']) for d in data]

    return {
        'student': {
        'points': torch.cat(student_points).float(),
        ...
        'teacher': {
        'points': torch.cat(teacher_points).float(),
        'batch_idx': torch.cat(b_idx).long(),
        'batch_size': batch_size,
    },
    }