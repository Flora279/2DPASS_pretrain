...
class point_image_dataset_semkitti(data.Dataset):
    def __init__(self, in_dataset, config, loader_config, num_vote=1, trans_std=[0.1, 0.1, 0.1], max_dropout_ratio=0.2):
        ...
        self.dropout = loader_config['dropout_aug']     # True
        ...
        self.coors_range_xyz = np.array([[self.min_volume_space[0], self.max_volume_space[0]],
                        [self.min_volume_space[1], self.max_volume_space[1]],
                        [self.min_volume_space[2], self.max_volume_space[2]]])
        self.pc_dataset_type = config['dataset_params']['pc_dataset_type']
        self.unlabeled_dropout_only = loader_config['unlabeled_dropout_aug']        # apply dropout to unlabeled points only
    ...
    def __getitem__(self, index):
        ...
        if self.dropout and self.point_cloud_dataset.imageset == 'train':
            dropout_ratio = np.random.random() * self.max_dropout_ratio     # [0,1) * 0.2
            drop_idx = np.where(np.random.random((xyz.shape[0])) <= dropout_ratio)[0]
            if self.pc_dataset_type == "SemanticKITTI_90fov_plabel" and self.unlabeled_dropout_only:
                print('unlabeled dropout')
                # apply dropout only to the unlabeled data (i.e. 270 fov, unmatched 90 fov pts)
                unlabeled_idx = np.where(labels == 0)[0]
                drop_idx = np.where(np.random.random((unlabeled_idx.shape[0])) <= dropout_ratio)[0]
                drop_idx = unlabeled_idx[drop_idx]

            if len(drop_idx) > 0:  # 90 plabel 训练时是不是应该去掉这个dropout？ 大概会取1%的data做dropout
                xyz[drop_idx, :] = xyz[0, :]
        ...
        xyz = np.clip(xyz, self.coors_range_xyz[:, 0], self.coors_range_xyz[:, 1])
