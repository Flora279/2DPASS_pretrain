...
class point_image_dataset_semkitti(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_x = np.logical_and(xyz_pol[:, 0] > self.min_volume_space[0], xyz_pol[:, 0] < self.max_volume_space[0])
        mask_y = np.logical_and(xyz_pol[:, 1] > self.min_volume_space[1], xyz_pol[:, 1] < self.max_volume_space[1])
        mask_z = np.logical_and(xyz_pol[:, 2] > self.min_volume_space[2], xyz_pol[:, 2] < self.max_volume_space[2])
