...
class point_encoder(nn.Module):
    ...
    def forward(self, features, data_dict):
        ...
        v_feat = torch_scatter.scatter_mean(
            ...
            dim=0
        )    # obtain current scale's voxel feature
...
class SPVBlock(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        # voxel encoder
        v_fea = self.v_enc(data_dict['sparse_tensor']) # 此时的sparse_tensor对应着self.last_scale
        ...
        # point encoder, 更新了coors & coors_inv & full_coors to current scale = self.scale
        p_fea = self.p_enc(
            features=data_dict['sparse_tensor'].features+v_fea.features,
            data_dict=data_dict
        )   # return current scale's voxel feature (updated)

        # fusion and pooling, voxel shape changes here
        data_dict['sparse_tensor'] = spconv.SparseConvTensor(
            features=p_fea+v_fea_inv, # N_vox, 64
            indices=data_dict['coors'],
            spatial_shape=self.spatial_shape,  # [1000,1000,60]
            batch_size=data_dict['batch_size']
        )   # 此时的sparse_tensor才对应着self.scale

        return p_fea[coors_inv]   # return current scla's point-wise feature
...
class get_model(pl.LightningModule):
    ...
    def forward(self, data_dict):
        ...
        data_dict = self.voxel_3d_generator(data_dict) # voxel feature data_dict['sparse_tensor'] 已经经过PPmodel-2MLP学习

        enc_feats = []
        for i in range(self.num_scales):
            enc_feats.append(self.spv_enc[i](data_dict))  # append point-wise features from each scale
