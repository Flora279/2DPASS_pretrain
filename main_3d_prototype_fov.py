...
from network.spvcnn_prototype import get_model as SPVCNN
from dataloader.pc_dataset import get_SemKITTI_label_name
from dataloader.dataset_3d import get_model_class, get_collate_class
from dataloader.pc_dataset_3d_fov import get_pc_model_class
...
class Backbone3DTrainer(pl.LightningModule):
    def __init__(self, config):
        ...
        num_class = config['model_params']['num_classes']
        embedding_size = self.hiden_size * self.num_scales        
        self.register_buffer('prototype', F.normalize(torch.rand(num_class-1, embedding_size), p=2, dim=1) ) # no need for ignore index
        self.register_buffer('momentum_coff', torch.tensor(0.99).type(torch.FloatTensor))
        self.register_buffer('momentum_coff_warm', torch.tensor(0.5).type(torch.FloatTensor))
    ...
    def forward(self, data_dict):
        # 3D network
        data_dict = self.model_3d(data_dict, self.prototype)
    ...
    def training_step(self, data_dict, batch_idx):
        ...
        step_embedding = data_dict['step_embedding']
        with torch.no_grad():
            if self.gpu_num > 1:
                step_embedding = self.all_gather(step_embedding).mean(axis=0)
            if self.global_step < 30:
                momentum_coff = self.momentum_coff_warm
            else:
                momentum_coff = self.momentum_coff

            self.prototype = momentum_coff*self.prototype+(1-momentum_coff)*step_embedding    
...
if __name__ == '__main__':
    ...
    if not configs['test']:
        ...
        os.system('cp main_3d_prototype_fov.py {}'.format(backup_dir))
        os.system('cp dataloader/dataset_3d.py {}'.format(backup_dir))
        os.system('cp dataloader/pc_dataset_3d_fov.py {}'.format(backup_dir))
        os.system('cp {} {}'.format(configs.config_path, backup_dir))
        # os.system('cp network/base_model.py {}'.format(backup_dir))
        os.system('cp network/spvcnn_prototype.py {}'.format(backup_dir))
