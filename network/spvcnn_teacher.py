...
class PartialConsistencyLoss(nn.Module):
    def __init__(self, num_scales, scale_list, ignore_index=0):
        super().__init__()
    ...
    def forward(self, student_dict, teacher_dict, student_label):
        mask = student_label == self.ignore_index
        loss = 0

        for idx in range(self.num_scales):
            loss += self.compute_consistency_loss(student_dict, teacher_dict, idx, mask=mask)

        return loss

    def compute_consistency_loss(self, student_dict, teacher_dict, idx, mask=None):
        last_scale = self.scale_list[idx - 1] if idx > 0 else 1
        student_coors_inv = student_dict['scale_{}'.format(last_scale)]['coors_inv']
        teacher_coors_inv = teacher_dict['scale_{}'.format(last_scale)]['coors_inv']

        student_pts_feat = student_dict['layer_{}'.format(idx)]['pts_feat'][student_coors_inv]
        teacher_pts_feat = teacher_dict['layer_{}'.format(idx)]['pts_feat'][teacher_coors_inv]

        cos_sim_loss = (1-F.cosine_similarity(student_pts_feat[mask], teacher_pts_feat[mask], dim=1)).mean()

        return cos_sim_loss

        # student_output_reshaped = student_pts_feat.log_softmax(1) 
        # teacher_output_reshaped = teacher_pts_feat.softmax(1)
...
#         mask = student_label == self.ignore_index
#         return self.compute_consistency_loss(student_output, teacher_output, mask=mask)
