...
class voxelization(nn.Module):
    ...
    def select_idx(self, coors, spatial_shape):
        print('self.coors_range_xy -50~50m?', self.coors_range_xy, coors.shape)   # coors_shape=N,2
        voxel_size = (self.coors_range_xy[:, 1] - self.coors_range_xy[:, 0]) / spatial_shape
        device = coors.device
        print('voxel_size', voxel_size)
        center_xy = (coors.float() + 0.5)*voxel_size.to(device) + self.coors_range_xy[:, 0].to(device)  # Add 0.5 to get the center of each voxel, -50m~50m
        print('coors', coors, self.coors_range_xy[:, 0], center_xy)
        center_xy_distance = torch.linalg.vector_norm(center_xy, dim=1)     # max dist = 70.71m
        print('max voxel dist', torch.max(center_xy_distance))
        select_30 = center_xy_distance[:] <= 30
        select_30to50 = (center_xy_distance[:] > 30) & (center_xy_distance[:] <= 50)
        select_50 = center_xy_distance[:] > 50      # 注意cylinder这里要改 cylinder最远就是50m
