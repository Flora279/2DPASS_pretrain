...
class SparseBasicBlock(spconv.SparseModule):
    ...
    def forward(self, x):
        ...
        output = self.layers(x)    # input & output spatial shape is the same
...
class ResNetFCN(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        layer1_out = self.layer1(self.maxpool(conv1_out)) # torch.Size([bs, 64, 160, 240])
        layer2_out = self.layer2(layer1_out) # torch.Size([bs, 128, 80, 120])
        layer3_out = self.layer3(layer2_out) # torch.Size([bs, 256, 40, 60])
        layer4_out = self.layer4(layer3_out) # torch.Size([bs, 512, 20, 30])
