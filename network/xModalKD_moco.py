...
class xModalKD(nn.Module):
    ...
    def fusion_to_single_KD(self, data_dict, idx):
        ...
        pts_feat_ = self.p2img_mapping(pts_feat[coors_inv], point2img_index, batch_idx)
        # KL divergence
        xm_loss = F.kl_div(
            F.log_softmax(pts_feat_, dim=1),
            F.softmax(img_feat.detach(), dim=1),
        )


        feat_learner = F.relu(self.leaners[idx](pts_feat)) # feat_learner -> voxel-wise feature after 2D learner
        # 3D prediction
        pts_pred_full = self.multihead_3d_classifier[idx]((pts_feat+feat_learner)) # pts_feat+feat_learner -> Enhanced 3D Features
        ...
        # KL divergence
        # xm_loss = F.kl_div(
        #     F.log_softmax(pts_feat, dim=1),
        #     F.softmax(fuse_feat.detach(), dim=1),
        # )
        # xm_loss = F.kl_div(
