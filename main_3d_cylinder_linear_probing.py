...
from network.spvcnn_cylinder import get_model as SPVCNN
...
class Backbone3DTrainer(pl.LightningModule):
    ...
    def training_step(self, data_dict, batch_idx):
        ...
        #    if name == 'model_3d.spv_enc.3.v_enc.1.layers.4.weight' or name == 'model_3d.spv_enc.2.p_enc.layer_out.2.weight' or ('classifier' in name):
        #       print('check if requires grads: name', name, 'param', param.requires_grad)
    ...
    def on_validation_epoch_end(self):
        ...
        if self.global_rank == 0:
            ...
            file = open(os.path.join(log_folder, configs.train_params.log_dir,
                                     'val_iou_{0}.txt'.format(configs['train_params']['base_lr'])), "a")
...
if __name__ == '__main__':
    ...
    if not configs['test']:
        ...
        shutil.copy('main_3d_cylinder.py', backup_dir)
        shutil.copy('network/voxel_fea_generator_cylinder.py', backup_dir)
        shutil.copy('dataloader/pc_dataset_3d.py', backup_dir)
        shutil.copy('dataloader/dataset_3d_cylinder.py', backup_dir)
        shutil.copy(configs.config_path, backup_dir)
        # os.system('cp network/base_model.py {}'.format(backup_dir))
        shutil.copy('network/spvcnn_cylinder.py', backup_dir)
    ...
    ckpt_path = '/public/home/zhangjy/weaksup/2DPASS/logs/SemanticKITTI/MAE_cylinder_scale_2_deconv2_20m40m_polar_feat/[0.9, 0.7, 0.5]/epoch=09-step=23910-val_MAE_loss=0.020.ckpt'  # mask=[0.9,0.7,0.5](0.688), [0.7,0.5,0.3](0.684)
    ...
    # 当只训练scale=2的时候pretrain ckpt也是可以全部load的，unused_parameters (not connected to loss) remains the same value during pretraining as in the initilization stage (where we also set seed, so we have exactly the same weights) 所以即使load classifier的weights也是没有问题的, check with gpu=0
    ...
    if configs["model_params"]["freeze_layers"]:     # linear probing, freeze backbone
        for name, param in my_model.named_parameters():
